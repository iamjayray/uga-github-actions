<template>
  <b-overlay
    ...
    variant="light-2"
    blur="3px"
    data-cy="undergrad-post-app-layout"
  >
    <template #overlay>
    ...
    <div id="ug-app-post-app" class="min-vh-100">
      <!-- skip to main content -->
      <div
        class="skip-link"
        data-cy="undergrad-post-app-layout-skip-to-main-content"
      >
...
export default {
  ...
  methods: {
    ...
    triggerErrorGtm(code, errors) {
      this.$track({
        event: "app_error",
        action: "layout_undergrad_post_app",
  ...
  async created() {
    // SkipStorePopulate parameter is sent from review page after app submission
    // if SkipStorePopulate is true we know that the user is comming from review page after app submission so skipping the store populate
    // Else all  the api will be executed and store will be populated
    if (!this.$route.params.SkipStorePopulate) {
